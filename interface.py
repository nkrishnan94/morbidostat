...
def read_in_data():
    ...
    # load the header and the last row into memory as a dataframe
        ...
        currOD = int(ODline[0][0])
        avOD = float(ODline[0][1])
        OD_timing = to_datetime(ODline[0][2])
        
        pump_nut = int(pumpline[0][0])
        pump_drug = int(pumpline[0][1])
        pump_waste = int(pumpline[0][2])
        pump_timing =  to_datetime(pumpline[0][3])
...
def prime_handler():
    ...
    subprocess.Popen('python equilibrate.py', shell=True)
